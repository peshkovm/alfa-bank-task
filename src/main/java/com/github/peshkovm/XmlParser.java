...
public class XmlParser {
  ...
  public XmlParser(ResourceLoader resourceLoader, String xmlRef) throws Exception {
    final Serializer serializer = new Persister();
    final var inputFile = resourceLoader.getResource(xmlRef).getFile();

    storageElement = serializer.read(StorageElement.class, inputFile);

    log.debug("storage = " + storageElement);
  }

  public Set<BoxElement> getBoxElements() {
    return getAllBoxes(storageElement.boxElements).collect(Collectors.toSet());
  }

  public Set<ItemElement> getItemElements() {
    return Stream.concat(
            storageElement.itemElements.stream(),
            getAllBoxes(storageElement.boxElements)
